...
template <class F> void BM_fastad(benchmark::State &state) {
  F f;
  ...
  for (auto _ : state) {
    grad_fx.setZero();
    ad::VarView<double, ad::vec> x_ad(x.data(), grad_fx.data(), x.size());
    auto expr = f(x_ad);
  ...
  std::cout << "FastAD val_buf bytesize: " << vb_size << std::endl;

  std::cout << "FastAD adj_buf bytesize: " << ab_size << std::endl;

  std::cout << "FastAD expr bytesize: " << e_size << std::endl;

  std::cout << "FastAD total bytesize: " << vb_size + ab_size + e_size
            << std::endl;
#endif
}
