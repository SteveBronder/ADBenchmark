...
struct MatrixProductFunc: MatrixProductFuncBase
{
    ...
    stan::math::var operator()(const stan::math::var_value<Eigen::Matrix<double, Eigen::Dynamic, 1>>& x) const
    {
        ...
        size_t N = std::sqrt(x.size() / 2);
        auto x1_val_arena = stan::math::to_arena(Eigen::Map<const mat_t>(x.val().data(), N, N));        
        auto x1_adj_arena = stan::math::to_arena(Eigen::Map<mat_t>(x.adj().data(), N, N));
        auto x2_val_arena = stan::math::to_arena(Eigen::Map<const mat_t>(x.val().data() + x.size()/2, N, N));
        auto x2_adj_arena = stan::math::to_arena(Eigen::Map<mat_t>(x.adj().data() + x.size()/2, N, N));
