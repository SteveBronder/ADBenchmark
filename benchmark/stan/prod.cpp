...
inline var prod2(T&& x) {
  ...
  return make_callback_var(val, [x_arena, val](auto& vi) mutable {
    ...
    if (zero_count == 0) {
      ...
      // Only the single zero receives gradient = upstream * product of the nonzeros.
      double prod_nonzero = 1.0;
      for (Eigen::Index i = 0; i < val_array.size(); ++i) {
        const double v = val_array(i);
        if (v != 0.0) prod_nonzero *= v;
      }
      // Add only to the zero position; others get 0.
      adj_array += zero_mask.select(Eigen::Matrix<double, -1, 1>::Constant(x_arena.rows(), x_arena.cols(), upstream * prod_nonzero), 0.0).array();
    } else {
