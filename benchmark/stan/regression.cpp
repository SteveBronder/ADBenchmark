...
struct RegressionFunc : RegressionFuncBase {
  ...
  operator()(const Eigen::Matrix<stan::math::var, Eigen::Dynamic, 1> &x) const {
    ...
    auto &b = x(N);
    auto &sigma = x(N + 1);
    return normal_lpdf(y, multiply(X, w) + multiply(b, vec_t::Ones(1000)),
                       sigma) +
           normal_lpdf(w, 0., 1.) + normal_lpdf(b, 0., 1.) -
  ...
  auto operator()(
    ...
    size_t N = (x.size() - 2);
    auto x_val_arena =
        stan::math::to_arena(Eigen::Map<const vec_t>(x.val().data(), N, N));
    auto x_adj_arena =
        stan::math::to_arena(Eigen::Map<vec_t>(x.adj().data(), N, N));
