...
struct StochasticVolatilityFuncBase : FuncBase {
  ...
  T operator()(Eigen::Matrix<T, Eigen::Dynamic, 1> &x) const {
    ...
    return normal_log_density(y.template cast<T>(), 0.,
                              (h / 2).array().exp().matrix()) +
           normal_log_density(h_std, 0., 1.) +
           cauchy_log_density(sigma, 0., 5.) + cauchy_log_density(mu, 0., 10.) -
           std::log(2.);
  }
  ...
  template <class T> auto operator()(ad::VarView<T, ad::vec> &x) const {
    ...
    auto tp =
        (h = h_std * sigma, h[0] /= ad::sqrt(1. - phi * phi), h += mu,
         ad::for_each(counting_iterator<>(1), counting_iterator<>(N),
  ...
  template <class T>
  auto normal_log_density(const Eigen::MatrixBase<T> &y, double mu,
                          double sigma) const {
  ...
  template <class T>
  T cauchy_log_density(const T &y, double loc, double scale) const {
    using std::log;
    T c = (y - loc) / scale;
