...
struct RegressionFuncBase : FuncBase {
  ...
  T operator()(const Eigen::Matrix<T, Eigen::Dynamic, 1> &x) const {
    ...
    auto &sigma = x(N + 1);
    auto z = y.template cast<T>() -
             ((X.template cast<T>() * w).array() + b(0)).matrix();
    return normal_log_density(z, 0., sigma) + normal_log_density(w, 0., 1.) +
           normal_log_density(b, 0., 1.) - std::log(10. - 0.1);
  }
  ...
  auto normal_log_density(const Eigen::MatrixBase<T> &y, const MType &mu,
                          const SType &sigma) const {
    using std::log;
    typename T::Scalar z_sq =
        (y.array() - mu).matrix().squaredNorm() / (sigma * sigma);
    return -0.5 * z_sq - y.size() * log(sigma);
  }

  template <class T, class MType, class SType>
  auto normal_log_density_scalar(const T &y, const MType &mu,
                                 const SType &sigma) const {
    using std::log;
  ...
  void derivative(Eigen::VectorXd &x, Eigen::VectorXd &grad) const {
    grad.setZero();
